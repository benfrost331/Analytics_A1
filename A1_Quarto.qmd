...
```{r, echo = FALSE}
# Regularised Elastic-Net Logistic Regression Tuning:
...
elasticNetResults <- lapply(alphaGrid, function(a) {  
  ...
  index <- which.min(abs(elasticNetFits$lambda - elasticNetFits$lambda.min)) #Smallest auroc for lambda value index
  
  coeficients <- coef(elasticNetFits, s = "lambda.min")
  n_nonzero <- sum(as.vector(coeficients)[-1] != 0) #number of predictors still not equal to zero
  ...
  list(
    ...
    summary = data.frame(alpha = a,
      ...
      n_nonzero = n_nonzero) #summary of results
})
  ...
  elasticNetModel <- glmnetUtils::cv.glmnet( 
    ...
    type.measure = "auc",
    trace.it = TRUE 
  ) #fitting model with best tuned parameter
...
```{r, echo = FALSE}
# Regularised Elastic-Net Logistic Regression Tuning Plots:
...
points(elasticNetResults$alpha[index_en], elasticNetResults$auc[index_en], pch = 19, col = "gold",) 

#optimal point label
text(x = elasticNetResults$alpha[index_en],
     y = y_bottom,
     labels = paste0(elasticNetResults$alpha[index_en]),
     pos = 1,      
     xpd = NA,
     cex = 0.75,
     col = "purple")
...
plot(elasticNetModel, ylab = "AUROC", cex.lab = 1.5)
```

```{r, echo = FALSE}
# Make HighHost a Factor:
...
if (file.exists(randomForestFile) && file.exists(rfResultsFile)) {
  ...
  #folds for cv
  fold <- sample(rep(1:10, length.out = nrow(trainData)))
...
rfResults <- lapply(1:nrow(rfGrid), function(i) {
  i_mtry     <- rfGrid$mtry[i]
  i_ntree    <- rfGrid$ntree[i]
  i_nodesize <- rfGrid$nodesize[i]
  ...
  for (k in 1:10) {
    ...
    train_fold <- trainData[fold != k, ]
    valid_fold <- trainData[fold == k, ]
  ...
  #save each hyperparameter 10-f cv result
  data.frame(
    mtry = i_mtry,
    ntree = i_ntree,
    nodesize = i_nodesize,
  ...
  #chosen best model after interpreting following plots
...
#plot results with point at chosen point
...
```{r, echo = FALSE}
# Variable Relative Importance Plot:
...
```{r, echo = FALSE}
# EN R Performance Metric Calculations:
#best lambda value by auc for our chosen alpha value
...
```{r, echo = FALSE}
# RF Performance Metric Calculations:
...
for (k in 1:10) {
  ...
  rf_outoffold_prob[fold == k] <- predict(
...
```{r, echo = FALSE}
# KNN Performance Metric Calculations:
...
```{r, echo = FALSE}
# GBM Performance Metric Calculations:
...
#display all metrics in table in quarto
ggplot(metric_long, aes(x = Metric, y = Value, fill = Model)) +
  ...
  theme(
    axis.title = element_text(size = 16),
    axis.text = element_text(size = 9.7, face = "bold"),
    legend.title = element_text(size = 16),
    legend.text = element_text(size = 14)
  )
...
```{r, echo = FALSE}
# Metric Calculation Given Tau Set Up:
...
ggplot(cvTauResults, aes(x = tau, y = F1.Score)) +
  ...
  theme(
    axis.title = element_text(size = 16),
    axis.text = element_text(size = 9.7),
    legend.title = element_text(size = 16),
    legend.text = element_text(size = 14)
  )
```
...
if (file.exists("pdp_plots.png")) {
  ...
  #turn plots into png for latex
  png("pdp_plots.png", width = 1200, height = 900, res = 150)
  
  #plot plots for quarto
  grid.arrange(
...
#categorise all different observations&predictions
...
#sample random FP and FN
FP_5 <- FP[sample(nrow(FP), 5), ]
FN_5 <- FN[sample(nrow(FN), 5), ]


#just looking at some basic trends in misclassifications
#mean(TP$price)
#mean(FP$price)
#mean(TN$price)
#mean(FN$price)
